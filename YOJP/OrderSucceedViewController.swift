...
//    }
    ...
    func creatCustomNavigationBar() {
        ...
        backBtn.addTarget(self, action: #selector(OrderSucceedViewController.backClicked), forControlEvents: UIControlEvents.TouchUpInside)
        backBtn.opaque = true
    ...
    func creatScrollView() {
        ...
        label.center = CGPointMake(screenWidth/2, 100+100)
        label.bounds = CGRectMake(0, 0, screenWidth, 200)
        label.text = "谢谢惠顾,购物愉快!"
        label.textAlignment = .Center
        label.font = font20
        self.scrollView.addSubview(label)
        
        
        let dateLabel : UILabel = UILabel(frame: CGRectMake(screenWidth/2+100-200,label.frame.origin.y+label.frame.size.height + 35,200,20))
        dateLabel.text = "取货时间：09:31   2015.12.16"
        dateLabel.textAlignment = .Right
        dateLabel.textColor = yojpText
        dateLabel.font = font14
        self.scrollView.addSubview(dateLabel)
        
        let payMoneyLabel : UILabel = UILabel(frame: CGRectMake(16,dateLabel.frame.origin.y+dateLabel.frame.size.height + 50,200,20))
